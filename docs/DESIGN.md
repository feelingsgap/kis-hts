...
- 2026-07-19: **통합 설정화면(⚙ 모달) 신규** — 상단 ⚙ → 중앙 모달 4구획. **A 연결·계정(편집 가능)**: 백엔드 `routers/settings.py`(`GET /settings/status`·`POST /settings/token/refresh`·`POST /settings/credentials`) + `TokenManager.settings_status/token_valid_until/refresh_token/apply_credentials`, `config.save_credentials(env, updates)`(config/{env}/.env 병합 저장·빈 칸 유지). 상태 표시(계좌/HTS ID/토큰 만료)와 자격증명 편집(App Key/Secret/계좌/HTS ID) — **보안: app_secret은 쓰기 전용(응답에 값 미반환, `has_secret`만), app_key는 마스킹(`_mask`)**. 토큰 재발급(캐시 삭제 후 재인증). 투자환경(모의↔실전) 전환은 kis_auth 전역상태라 백엔드 재시작 필요(모달에 안내만). **B 주문 기본값**: 기본 유형·확인창 on/off·수량 프리셋(%) 편집(`OrderPanel`이 소비, **실전(prod)은 확인창 강제**). **C 화면**: 등락 색상 한국식(red/blue)↔글로벌식(green/red) — `:root[data-scheme]` CSS var 교체, App이 `<html data-scheme>` 설정. **D 알림**: 체결 토스트·소리(`sound.ts` WebAudio beep) on/off, 가격 알림 전체목록 중앙 삭제. 상태는 `settings.ts`(zustand+localStorage `kis-hts:settings`), `store.env`에 서버 env 반영. 검증: 빌드/lint 통과, 백엔드 status가 secret 미반환·app_key 마스킹 확인, 브라우저에서 모달 4구획 렌더·색상 전환 라이브 적용·리로드 후 영속 확인. **미검증(사용자 몫)**: 자격증명 저장(실동작 .env 덮어씀 방지로 미제출)·토큰 재발급 실호출.
