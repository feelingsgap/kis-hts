...
- **모든 KIS REST 호출은 `kis/rest.py`의 단일 직렬 rate limiter를 거친다**(`_limiter.wait()`, `vps 0.55s / prod 0.07s`, 실측 튜닝). 버스트는 `EGW00201`(초당 초과)을 유발한다. 다중 호출 흐름이 느린 건 이 제약 때문이지 버그가 아니다.
